...
// MQTT Broker Details
const char* mqtt_server = "192.168.17.207";
...
String getDrinkName(Mocktail drink) {
  switch(drink) {
    case GRAPE_FIZZ: return "Grape Fizz";
    case ORANGE_FIZZ: return "Orange Fizz";
    case PURPLE_SUNSET: return "Purple Sunset";
    case ORANGE_ON_TOP: return "Orange on Top";
    case GRAPE_ON_TOP: return "Grape on Top";
    default: return "Unknown Drink";
  }
}
...
void Fire2012WithPalette() {
  ...
  if (millis() - lastUpdate < FLAME_SPEED) return;
  lastUpdate = millis();
...
void startDispensingProcess() {
  if (totalSteps == 0 || motorState != IDLE) return;
...
void callback(char* topic, byte* payload, unsigned int length){
  ...
  if (String(topic) == drink_selection_topic) {
    ...
    if (drinkIndex == 0 && message[0] != '0') {
        ...
        if (drinkName == "grape_fizz") drinkIndex = 0;
        else if (drinkName == "orange_fizz") drinkIndex = 1;
        else if (drinkName == "purple_sunset") drinkIndex = 2;
        else if (drinkName == "orange_on_top") drinkIndex = 3;
        else if (drinkName == "grape_on_top") drinkIndex = 4;
    }
    ...
    if (drinkIndex >= 0 && drinkIndex <= 4) {
        ...
        if ((currentDrink == GRAPE_FIZZ || currentDrink == ORANGE_FIZZ || 
             ...
             currentDrink == GRAPE_ON_TOP) && !spriteAvailable) {
